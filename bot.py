...
def deposit_method_keyboard() -> InlineKeyboardMarkup:
    ...
    add_button(kb, text="🪙 USDT BEP-20", callback_data="dep_method:usdt_bep20", style="success")
...
def withdraw_method_keyboard() -> InlineKeyboardMarkup:
    ...
    add_button(kb, text="🪙 USDT BEP-20", callback_data="with_method:usdt_bep20", style="success")
    add_button(kb, text="⬅️ Back", callback_data="wallet:back", style="danger")
    kb.adjust(2, 1)
    return kb.as_markup()
...
def gateway_keyboard() -> InlineKeyboardMarkup:
    ...
    add_button(kb, text="🟢 Set UPI QR", callback_data="gateway:upi_qr", style="success")
    add_button(kb, text="🟢 Set USDT Address", callback_data="gateway:usdt", style="success")
    add_button(kb, text="🟢 Set USDT QR", callback_data="gateway:usdt_qr", style="success")
    add_button(kb, text="🔵 Set USD/INR Rate", callback_data="gateway:rate", style="primary")
    add_button(kb, text="🔵 View Payment Settings", callback_data="gateway:view", style="primary")
    add_button(kb, text="🔴 Close", callback_data="gateway:close", style="danger")
    kb.adjust(2, 2, 2, 1)
    return kb.as_markup()
...
async def admin_set_upi_prompt(call: CallbackQuery, state: FSMContext):
    ...
    await call.answer()
    await call.message.answer("Enter the platform UPI ID:", reply_markup=ForceReply(selective=True))
...
async def admin_set_usdt_prompt(call: CallbackQuery, state: FSMContext):
    if not admin_only(call.from_user.id):
        await call.answer("Unauthorized.", show_alert=True); return
    await call.answer()
    await call.message.answer("Enter the platform USDT BEP-20 receiving address:", reply_markup=ForceReply(selective=True))
