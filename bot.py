...
def withdraw_method_keyboard() -> InlineKeyboardMarkup:
    ...
    add_button(kb, text="📷 QR Code", callback_data="with_method:qr", style="primary")
    add_button(kb, text="⬅️ Back", callback_data="wallet:back", style="primary")
    kb.adjust(2, 1, 1)
    return kb.as_markup()
...
def get_admin_menu_keyboard():
    ...
    status_text = "⚡ Bot Status: ON" if BOT_STATUS else "⏸️ Bot Status: OFF"
...
async def notify_admins_new_tx(tx_id, tx_type, user, method, amount, details=None):
    ...
    # Send uploaded payment/recipient QR proof as a separate real-time admin message.
    if tx and tx["details"]:
        proof = re.search(r"(?:^|;)proof_file_id=([^;]+)", tx["details"])
        qr_proof = re.search(r"(?:^|;)recipient_qr_file_id=([^;]+)", tx["details"])
        file_id = proof.group(1) if proof else (qr_proof.group(1) if qr_proof else None)
        if file_id:
            caption = f"🧾 Deposit proof — Tx #{tx_id}" if proof else f"📷 Withdrawal QR — Tx #{tx_id}"
            for admin_id in ADMIN_IDS:
                try:
...
async def process_deposit_amount(message: Message, state: FSMContext):
    ...
    if method == "upi":
        ...
        if not usdt_address or usdt_address.startswith("0x000000"):
            ...
            return
        upi_line = f"🏦 UPI ID: <code>{upi_id}</code>\n" if upi_id and upi_id != "not_set@upi" else ""
        text = (
...
async def process_deposit_utr(message: Message, state: FSMContext):
    ...
    await show_wallet(message)
    asyncio.create_task(notify_admins_new_tx(tx_id, "deposit", user, method, amount))
...
async def process_with_amount(message: Message, state: FSMContext):
    ...
    if method == "qr":
        await message.answer(
            "📷 Send the recipient payment QR code as a photo.\n\n"
            "The QR will be sent to the admin with your withdrawal request."
        )
        await state.set_state(WalletStates.with_qr)
        return
...
@router.message(WalletStates.with_qr, F.photo)
async def process_with_qr(message: Message, state: FSMContext):
    user = message.from_user
    data = await state.get_data()
    ...
    with closing(get_conn()) as conn:
        ...
        details = f"recipient_qr_file_id={qr_file_id}"
        cur = conn.execute(
            "INSERT INTO transactions (user_id, tx_type, method, amount, details, status, created_at) VALUES (?, 'withdrawal', ?, ?, ?, 'pending', ?)",
            (user.id, "qr", amount, details, datetime.utcnow().isoformat()),
        )
        conn.commit()
        tx_id = cur.lastrowid
    ...
    await state.clear()
    await message.answer(
        f"✅ <b>QR withdrawal request submitted.</b>\n\nTx ID: #{tx_id}\nAmount: ${amount}\n\n"
        "The admin will verify the QR code and process the withdrawal.",
        parse_mode=ParseMode.HTML,
    )
    await show_wallet(message)
    asyncio.create_task(notify_admins_new_tx(tx_id, "withdrawal", user, "qr", amount, details))
...
@router.message(F.text.in_({"⚡ Bot Status: ON", "⏸️ Bot Status: OFF"}))
async def admin_toggle_bot_status(message: Message):
    ...
    await message.answer(
        f"Bot status is now: {'⚡ ON' if BOT_STATUS else '⏸️ OFF'}",
        reply_markup=get_admin_menu_keyboard(),
    )
...
async def admin_view_gateway_settings(call: CallbackQuery):
    ...
    await call.message.answer(
        ...
        f"🖼️ UPI QR: {'Configured' if upi_qr else 'Not configured'}\n"
        f"🪙 USDT address: <code>{usdt}</code>\n"
        f"🖼️ USDT QR: {'Configured' if usdt_qr else 'Not configured'}\n"
...
async def handle_tx_approval(call: CallbackQuery):
    ...
    asyncio.create_task(notify_user_tx_result(tx_result))
