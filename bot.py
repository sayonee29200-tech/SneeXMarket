...
f"Tx ID: #{tx['tx_id']}\n"
f"Amount: latex
{tx['amount']}\n\n" f"{body}" ) else: body = ( "The reserved amount has been refunded to your wallet balance." if tx["tx_type"] == "withdrawal" else "If you believe this is a mistake, please contact support." ) text = ( f"❌ &lt;b&gt;{tx_type_label} Declined&lt;/b&gt;\n\n" f"Tx ID: #{tx['tx_id']}\n" f"Amount: 
...
f"💵 USD deposit: <b>latex
{amount}&lt;/b&gt;\n" f"💱 Admin rate: &lt;b&gt;₹{rate:g} / 
...
try:
raw = (message.text or "").strip().replace("$", "") amount = int(raw) if amount &lt; MIN_WITHDRAWAL: raise ValueError except ValueError: await message.answer(f"❌ Minimum withdrawal is ${MIN_WITHDRAWAL}.\nEnter a valid dollar amount.")
return
